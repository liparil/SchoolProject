...
namespace Schoole.Services.CourseService
{
    ...
    {
        Task<Output> Execute(string title, int teacherId);
    }
    public class AddCourse(ITeacherRepository teacherRepository, ICourseRepository courseRepository, ILogService logService) : IAddCourse
    {

        public async Task<Output> Execute(string title, int teacherId)
        {
            ...
            var newCourse = new Course { Title = title, TeacherId = teacherId };
